...
class JimengVideo:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    # 将所有session用逗号拼接，后端会自动选择积分>=45的
                    all_sessions = ','.join(sessions)
                    print(f"✅ 读取号池成功（共{pool_size}个Session，后端将自动选择积分>=45分的）")
                    return all_sessions
                else:
    ...
    def generate_video(self, api_url, session_id, prompt, model, width, height, 
                      resolution, first_frame=None, end_frame=None):
        """生成视频"""
    ...
    def generate(self, prompt, model, aspect_ratio, resolution,
        ...
        try:
            ...
            video_url = self.generate_video(
                ...
                resolution, first_frame, end_frame
            )
